...
{
    public partial class AdditionalTimeDetails : BasePage
    {
                ...
                try
                {
                    ...
                    if (matterReturnValue.Success)
                    {
                        if (matterReturnValue != null)
                        {
                            ...
                            {
                                _cliMatDetailsFileReview.MemberId = matterReturnValue.ClientDetails.MemberId;
                                _cliMatDetailsFileReview.OrganisationId = DataConstants.DummyGuid;
                            }
                            else
                            {
                                _cliMatDetailsFileReview.MemberId = DataConstants.DummyGuid;
                                _cliMatDetailsFileReview.OrganisationId = matterReturnValue.ClientDetails.MemberId;
                            }

                            _cliMatDetailsFileReview.IsClientMember = matterReturnValue.ClientDetails.IsMember;
                            _cliMatDetailsFileReview.ClientRef = matterReturnValue.ClientDetails.Reference;
                            _cliMatDetailsFileReview.ClientName = matterReturnValue.ClientDetails.FullName;
                            
                            if (_cliMatDetailsFileReview.Message != null)
                            {
                                if (_cliMatDetailsFileReview.Message.Trim().Length > 0)
                                {
                                    throw new Exception("Loading failed for Client Matter Details.<br>Exception occured is: " + _cliMatDetailsFileReview.Message);                                    
                                }
                            }
        ...
        #region _cliMatDetailsFileReview_MatterChanged
        /// <summary>
        ...
        /// <param name="e"></param>
        protected void _cliMatDetailsFileReview_MatterChanged(object sender, EventArgs e)
        {
            try
            {
                ViewState["FileReviewProjectId"] = _cliMatDetailsFileReview.ProjectId.ToString();
                if (_cliMatDetailsFileReview.ProjectId == DataConstants.DummyGuid)
                {
                    if (_cliMatDetailsFileReview.Message != null)
                    {
                        if (_cliMatDetailsFileReview.Message.Trim().Length > 0)
                        {
                            _lblError.CssClass = "errorMessage";
                            _lblError.Text = _cliMatDetailsFileReview.Message;
                            return;
                        }
                    }
                }
